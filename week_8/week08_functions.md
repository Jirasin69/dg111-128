...
**คำตอบ:`เพราะถูก clamp กำหนดใว้ที่ 100**

### 2.2 `lerp` — Local Scope

**คำถาม:** ทำไม `main()` ถึงมองไม่เห็นตัวแปร `result` ทั้งที่มันถูกประกาศไว้ในไฟล์เดียวกัน?

**คำตอบ: result โดนประกาศใน lerp() ทำให้เป็น Local Variableเลยเลียกใช่โดยตรงไม่ได้**
...
**คำตอบ:Pass by Value ส่ง diceMin เข้าฟังชั้นเป็นการคัดลอกเฉพาะค่าไปให้ `min` ไปแก้ไขค่า**
...
**คำตอบ:Pass by Value การแก้ค่า `current` ใน `percentOf()` เป็นการแก้ไขตัวแปรสำเนาในฟังชั้น เลยไม่มีผลกับ `score`**
